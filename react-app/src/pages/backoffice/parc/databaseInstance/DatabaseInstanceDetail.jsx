...
const DatabaseInstanceDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        ...
        const [itemData, documentsData] = await Promise.all([
          DatabaseInstanceService.getDatabaseInstanceById(id),
          DocumentService.getDocumentsForItem('DatabaseInstance', id)
        ]);
        setItem(itemData);
  ...
  if (loading) {
    return (
      <div className="ticket-detail-page">
        <div className="loading-state">Chargement de la base de données...</div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="ticket-detail-page">
        <div className="empty-state">Base de données introuvable</div>
      </div>
    );
  }
  ...
  return (
    <div className="ticket-detail-page">
      <div className="detail-header">
        <Link to="/parc/databaseInstances" className="back-btn">
          <ArrowLeft size={16} />
          Retour à la liste
        </Link>
        <h1>Base de données #{item.id} - {item.name || 'Sans titre'}</h1>
      </div>
...
export default DatabaseInstanceDetail;
