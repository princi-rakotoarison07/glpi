...
const CartridgeDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        ...
        const [itemData, documentsData] = await Promise.all([
          CartridgeItemService.getCartridgeItemById(id),
          DocumentService.getDocumentsForItem('CartridgeItem', id)
        ]);
        setItem(itemData);
  ...
  if (loading) {
    return (
      <div className="ticket-detail-page">
        <div className="loading-state">Chargement de la cartouche...</div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="ticket-detail-page">
        <div className="empty-state">Cartouche introuvable</div>
      </div>
    );
  }
  ...
  return (
    <div className="ticket-detail-page">
      <div className="detail-header">
        <Link to="/parc/cartridges" className="back-btn">
          <ArrowLeft size={16} />
          Retour à la liste
        </Link>
        <h1>Cartouche #{item.id} - {item.name || 'Sans titre'}</h1>
      </div>
