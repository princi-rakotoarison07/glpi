...
const CartridgeList = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const data = await CartridgeItemService.getAllCartridgeItems();
      setItems(data || []);
    } catch (error) {
                ...
                items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="ticket-title">
                      {item.name || `Cartouche #${item.id}`}
                    </td>
                    <td>{formatDate(item.date_creation || item.date)}</td>
                    <td>{formatDate(item.date_mod)}</td>
                    <td className="action-cell">
                      <Link to={`/parc/cartridges/${item.id}`} className="action-btn view-btn">
                        <Eye size={16} />
                        Détails
                      </Link>
                    </td>
