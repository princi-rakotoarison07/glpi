...
const ConsumableList = () => {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const data = await ConsumableItemService.getAllConsumableItems();
      setItems(data || []);
    } catch (error) {
                ...
                items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="ticket-title">
                      {item.name || `Consommable #${item.id}`}
                    </td>
                    <td>{formatDate(item.date_creation || item.date)}</td>
                    <td>{formatDate(item.date_mod)}</td>
                    <td className="action-cell">
                      <Link to={`/parc/consumables/${item.id}`} className="action-btn view-btn">
                        <Eye size={16} />
                        Détails
                      </Link>
                    </td>
