...
const DCRoomDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        ...
        const [itemData, documentsData] = await Promise.all([
          DCRoomService.getDCRoomById(id),
          DocumentService.getDocumentsForItem('DCRoom', id)
        ]);
        setItem(itemData);
        setDocuments(documentsData || []);
      } catch (error) {
        console.error('Error fetching salle serveur:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="ticket-detail-page">
        <div className="loading-state">Chargement de la salle serveur...</div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="ticket-detail-page">
        <div className="empty-state">Salle serveur introuvable</div>
      </div>
    );
  }
  ...
  return (
    <div className="ticket-detail-page">
      <div className="detail-header">
        <Link to="/parc/dcRooms" className="back-btn">
          <ArrowLeft size={16} />
          Retour à la liste
        </Link>
        <h1>Salle serveur #{item.id} - {item.name || 'Sans titre'}</h1>
      </div>
