...
const PassiveEquipmentDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        ...
        const [itemData, documentsData] = await Promise.all([
          PassiveDCEquipmentService.getPassiveDCEquipmentById(id),
          DocumentService.getDocumentsForItem('PassiveDCEquipment', id)
        ]);
        setItem(itemData);
        setDocuments(documentsData || []);
      } catch (error) {
        console.error('Error fetching passive equipment:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="ticket-detail-page">
        <div className="loading-state">Chargement du matériel passif...</div>
      </div>
    );
  }
  ...
  if (!item) {
    return (
      <div className="ticket-detail-page">
        <div className="empty-state">Matériel passif introuvable</div>
      </div>
    );
  }
  ...
  return (
    <div className="ticket-detail-page">
      <div className="detail-header">
        <Link to="/parc/passiveEquipment" className="back-btn">
          <ArrowLeft size={16} />
          Retour à la liste
        </Link>
        <h1>Matériel passif #{item.id} - {item.name || 'Sans titre'}</h1>
      </div>
...
export default PassiveEquipmentDetail;
